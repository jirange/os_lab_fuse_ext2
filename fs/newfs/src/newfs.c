...
struct newfs_super newfs_super; //原来是super
/******************************************************************************
* SECTION: FUSE操作定义
*******************************************************************************/
...
int newfs_getattr(const char* path, struct stat * newfs_stat) {
	/* TODO: 解析路径，获取Inode，填充newfs_stat，可参考/fs/simplefs/sfs.c的newfs_getattr()函数实现 */
	//​ ①首先找到路径所对应的目录项。
	// ②判断目录项的文件类型并对状态进行编写。
	boolean	is_find, is_root;
	...
	if (is_root) {
		newfs_stat->st_size	= newfs_super.sz_usage; // 这个对吗 是不是应该是0啊
		// st_size	文件的大小
		newfs_stat->st_blocks = NEWFS_DISK_SZ() / NEWFS_BLK_SZ();// 应该改为逻辑块 之前是IO块
...
int newfs_readdir(const char * path, void * buf, fuse_fill_dir_t filler, off_t offset,
    ...
    /* TODO: 解析路径，获取目录的Inode，并读取目录项，利用filler填充到buf，可参考/fs/simplefs/sfs.c的newfs_readdir()函数实现 */
    boolean	is_find, is_root;
	int		cur_dir = offset;
